...
pub async fn run(config: Config) -> Result<(), Box<dyn Error>> {
    ...
    info!("Reading input from {}", config.input.display());
    let input = tokio::fs::read_to_string(&config.input).await?;
    info!("Using backend {}", config.backend.as_str());
    ...
    match config.backend {
        ...
        Backend::GnuC => {
            ...
            let total_batches = (domains.len() + concurrency.saturating_sub(1)) / concurrency;
            info!(
            ...
            for batch_idx in 0..total_batches {
                ...
                join_set.spawn_blocking(move || {
                    let resolved =
                        gnu_c_backend::resolve_domains_gnu_c(&domains[start..end], ipv4_only);
                    (start, resolved)
                });
            }
    ...
    tokio::fs::write(&config.output, output).await?;
